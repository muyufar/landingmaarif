<?php

declare(strict_types=1);

/** @var array $rows @var string $search @var string $statusFilter */
?>
<div class="bg-white rounded-2xl shadow-lg border border-green-100 overflow-hidden">
  <div class="px-6 py-5 border-b border-gray-100">
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mb-4">
      <div>
        <h2 class="text-xl font-bold text-green-800">Kelola Berita</h2>
        <p class="text-sm text-gray-500 mt-1">Total: <strong><?= count($rows) ?></strong> berita</p>
      </div>
      <a href="<?= url('admin/?page=berita-form') ?>"
         class="bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-lg text-sm font-medium text-center">+ Tulis Berita</a>
    </div>

    <form method="get" class="grid grid-cols-1 sm:grid-cols-3 gap-3">
      <input type="hidden" name="page" value="berita">
      <input type="text" name="q" value="<?= sanitize($search) ?>" placeholder="Cari judul / isi berita..."
             class="rounded-lg border border-gray-300 px-3 py-2 text-sm sm:col-span-2 focus:ring-2 focus:ring-green-600">
      <select name="status" class="rounded-lg border border-gray-300 px-3 py-2 text-sm">
        <option value="">Semua Status</option>
        <option value="published" <?= $statusFilter === 'published' ? 'selected' : '' ?>>Terbit</option>
        <option value="draft" <?= $statusFilter === 'draft' ? 'selected' : '' ?>>Draft</option>
      </select>
      <div class="flex gap-2 sm:col-span-3">
        <button type="submit" class="bg-green-700 hover:bg-green-800 text-white px-4 py-2 rounded-lg text-sm font-medium">Filter</button>
        <a href="<?= url('admin/?page=berita') ?>" class="bg-gray-200 hover:bg-gray-300 text-gray-800 px-4 py-2 rounded-lg text-sm font-medium">Reset</a>
      </div>
    </form>
  </div>

  <?php if (empty($rows)): ?>
    <div class="px-6 py-16 text-center text-gray-500">Belum ada berita.</div>
  <?php else: ?>
    <div class="overflow-x-auto">
      <table class="w-full text-sm">
        <thead class="bg-green-50 text-green-900 text-xs uppercase">
          <tr>
            <th class="px-4 py-3 text-left">No</th>
            <th class="px-4 py-3 text-left">Judul</th>
            <th class="px-4 py-3 text-left">Status</th>
            <th class="px-4 py-3 text-left">Tanggal</th>
            <th class="px-4 py-3 text-center">Aksi</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-gray-100">
          <?php foreach ($rows as $index => $row): ?>
            <tr class="hover:bg-gray-50">
              <td class="px-4 py-3 text-gray-500"><?= $index + 1 ?></td>
              <td class="px-4 py-3">
                <div class="flex items-start gap-3">
                  <?php if (!empty($row['gambar'])): ?>
                    <img src="<?= url($row['gambar']) ?>" alt="" class="w-14 h-14 object-cover rounded-lg border border-gray-200 shrink-0">
                  <?php endif; ?>
                  <div class="min-w-0">
                    <p class="font-semibold"><?= sanitize($row['judul'] ?? '') ?></p>
                    <p class="text-xs text-gray-500 mt-0.5 line-clamp-2"><?= sanitize(ringkasanBerita($row, 100)) ?></p>
                  </div>
                </div>
              </td>
              <td class="px-4 py-3">
                <?php if (($row['status'] ?? '') === 'published'): ?>
                  <span class="bg-green-100 text-green-800 px-2 py-1 rounded text-xs font-semibold">Terbit</span>
                <?php else: ?>
                  <span class="bg-amber-100 text-amber-800 px-2 py-1 rounded text-xs font-semibold">Draft</span>
                <?php endif; ?>
              </td>
              <td class="px-4 py-3 whitespace-nowrap text-gray-600">
                <?= sanitize(formatTanggalBerita($row['published_at'] ?? $row['created_at'] ?? null)) ?>
              </td>
              <td class="px-4 py-3">
                <div class="flex items-center justify-center gap-2 flex-wrap">
                  <?php if (($row['status'] ?? '') === 'published'): ?>
                    <a href="<?= url('berita/?slug=' . urlencode($row['slug'] ?? '')) ?>" target="_blank"
                       class="text-green-700 hover:underline text-xs font-medium">Lihat</a>
                  <?php endif; ?>
                  <a href="<?= url('admin/?page=berita-form&id=' . (int) ($row['id'] ?? 0)) ?>"
                     class="text-blue-700 hover:underline text-xs font-medium">Edit</a>
                  <form method="post" class="inline" onsubmit="return confirm('Hapus berita ini?');">
                    <input type="hidden" name="delete_berita_id" value="<?= (int) ($row['id'] ?? 0) ?>">
                    <button type="submit" class="text-red-600 hover:underline text-xs font-medium">Hapus</button>
                  </form>
                </div>
              </td>
            </tr>
          <?php endforeach; ?>
        </tbody>
      </table>
    </div>
  <?php endif; ?>
</div>
