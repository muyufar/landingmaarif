...
function pemesananNavClass(string $page, string $current): string
{
    return $page === $current
...
<head>
  ...
  <title><?= sanitize($pageTitle ?? 'Admin Pemesanan Buku') ?> | LP Ma'arif NU Magelang</title>
    ...
    <div class="max-w-[1400px] mx-auto px-4 sm:px-6 py-4">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div class="flex items-center space-x-4">
          ...
          <div>
            <h1 class="text-lg md:text-xl font-bold">Admin Pemesanan Layanan</h1>
            <p class="text-sm text-green-100">MOPDIK, Batik, Buku Ke-NU-an, Buku Aswaja</p>
          </div>
        </div>
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <?php if (isPemesananAdminLoggedIn()): ?>
          <nav class="flex rounded-lg overflow-hidden border border-green-700 text-sm font-medium">
            <a href="<?= url('adminpemesananbuku/?page=dashboard') ?>"
               class="px-4 py-2 transition <?= pemesananNavClass('dashboard', $currentPage ?? '') ?>">Dashboard</a>
            <a href="<?= url('adminpemesananbuku/?page=list') ?>"
               class="px-4 py-2 transition <?= pemesananNavClass('list', $currentPage ?? '') ?>">List Pemesanan</a>
          </nav>
          <a href="<?= url('adminpemesananbuku/?logout=1') ?>"
