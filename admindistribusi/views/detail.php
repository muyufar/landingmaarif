<?php declare(strict_types=1); /** @var array $satuan @var array $totalTerima @var array $pengirimanList @var array|null $pengkinian */ ?>
<?php $kurang = satuanKurangDetail($satuan, $totalTerima); ?>
  ...
  <table class="w-full text-sm border">
    ...
    <tbody>
      <?php for ($i = 1; $i <= 6; $i++):
        ...
        $got = $totalTerima[$i] ?? 0;
      ?>
        <tr class="border-t"><td class="px-3 py-2">Kelas <?= $i ?></td><td class="px-3 py-2 text-center"><?= $need ?></td><td class="px-3 py-2 text-center"><?= $got ?></td><td class="px-3 py-2 text-center text-amber-700 font-semibold"><?= max(0, $need - $got) ?></td></tr>
      <?php endfor; ?>
    </tbody>
  </table>
</div>
